...
class Coit: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let url = URL(string: "http://www.elranchoinc.com/el/weeklyad/el%2014/p1.jpg")
    ...
    @IBAction func pageTwo(_ sender: AnyObject) {
        let url = URL(string: "http://www.elranchoinc.com/el/weeklyad/el%2014/p2.jpg")
        let request = URLRequest(url: url!)
        WebView.loadRequest(request)
    }
    
    @IBAction func pageThree(_ sender: AnyObject) {
        let url = URL(string: "http://www.elranchoinc.com/el/weeklyad/el%2014/p3.jpg")
        let request = URLRequest(url: url!)
        WebView.loadRequest(request)
    }
    @IBAction func pageFour(_ sender: AnyObject) {
        let url = URL(string: "http://www.elranchoinc.com/el/weeklyad/el%2014/p4.jpg")
        let request = URLRequest(url: url!)
        WebView.loadRequest(request)
    }
